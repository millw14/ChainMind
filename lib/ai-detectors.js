...
import { baselineRowForZScores } from "./baseline-manager.js";
 ...
 *  v1 labels (swap_eligible, spl) kept for un-upgraded rows; v2 labels (parse-tx.js)
 *  added so detection works on mixed data during/after the classifier rollout. Excludes
 *  pure sol_transfer / other / unknown — not token-trade activity. */
const TRADE_LIKE_EVENT_TYPES = new Set([
  "swap_eligible",
  "spl",
  "swap",
  "transfer",
...
export function ledgerCohortYouthNorm01(w) {
  ...
  for (const r of w.rows ?? []) {
    // History-capped ageDays is a window edge, not a birth date — skip for median.
    if (r.historyCapped === true) continue;
...
export function detectWashRotation(transfers, scopeMint, participantSet) {
  ...
  // Wash trading = repeated, value-BALANCED round-trips (volume with ~zero net flow).
  // Require enough legs each way AND near-balanced summed value, so ordinary two-way
  // activity between active wallets (a few transfers, or lopsided amounts) is not flagged.
  const MIN_LEG_EACH = 2; // ≥2 transfers in each direction (was ≥1)
  const MIN_LEGS_TOTAL = 6; // ≥6 total legs (was ≥3)
  const MAX_NET_IMBALANCE = 0.2; // |out−in| / max(out,in) must be ≤ 0.2 (balanced)
  /** @type {Map<string, number>} */
  const dirCount = new Map();
  /** @type {Map<string, bigint>} */
  const dirAmount = new Map();
  ...
  for (const t of transfers) {
    ...
    // A known entity (CEX/market-maker/launchpad/infra) on either side isn't the
    // token's wash trading — its round-tripping is its own business.
    if (isKnownEntity(from) || isKnownEntity(to)) continue;
    ...
    dirAmount.set(kk, (dirAmount.get(kk) ?? 0n) + toBig(t.amount));
  }

  /** @type {{ pair: [string, string], forward: number, back: number, mint: string, imbalance: number }[]} */
  const suspicious = [];
  ...
  for (const [k] of dirCount) {
    ...
    if (fLeg < MIN_LEG_EACH || bLeg < MIN_LEG_EACH || fLeg + bLeg < MIN_LEGS_TOTAL) continue;

    // Value balance — the defining wash signature. Lopsided value = real flow, not wash.
    const fAmt = dirAmount.get(dkey(x, y, asset)) ?? 0n;
    const bAmt = dirAmount.get(dkey(y, x, asset)) ?? 0n;
    const hi = fAmt > bAmt ? fAmt : bAmt;
    const lo = fAmt > bAmt ? bAmt : fAmt;
    // hi === 0 means zero-amount legs (no value info) — don't claim wash on that alone.
    if (hi === 0n) continue;
    const imbalance = Number(hi - lo) / Number(hi);
    if (imbalance > MAX_NET_IMBALANCE) continue;

    suspicious.push({ pair: [x, y], forward: fLeg, back: bLeg, mint: asset, imbalance });
  }
  ...
  return {
    ...
    triggered,
    severity: triggered ? (suspicious.length >= 2 || suspicious[0].forward + suspicious[0].back >= 10 ? "high" : "medium") : "none",
    summary: triggered
      ? (() => {
          ...
          return `Balanced reciprocal transfers (${s0.forward}+${s0.back} legs, ${(s0.imbalance * 100).toFixed(0)}% net imbalance) between linked wallets on ${mLabel} — possible wash rotation.`;
        })()
      : "No balanced high-frequency two-way transfer legs among watched wallets for this mint in window.",
    evidence: suspicious.slice(0, 4).map((s) => ({
      ...
      netImbalancePct: Math.round(s.imbalance * 100),
      mintShort:
 ...
 *   storedBaseline?: Record<string, unknown> | null,
 * }} opts
 */
export function runAiDetectors(opts) {
  const { scope, eventRows, scoreResult, fundingGraph, transfers, peerEdges, walletLedgerAge, storedBaseline } = opts;
  ...
  const baselineForZ = baselineRowForZScores(storedBaseline ?? null);
  const zScoreAll = computeZScores(timelineBuckets, baselineForZ);
  const z =
    zScoreAll.available && zScoreAll.peakEventZ != null && Number.isFinite(zScoreAll.peakEventZ)
      ? zScoreAll.peakEventZ
      : 0;
  ...
  const zScoreTrade = computeZScores(tradeBuckets, null);
  const zTrade =
    zScoreTrade.available && zScoreTrade.peakEventZ != null && Number.isFinite(zScoreTrade.peakEventZ)
      ? zScoreTrade.peakEventZ
      : null;
  const buyBurstTradeNorm =
    zTrade != null && zScoreTrade.available ? Math.min(1, Math.max(0, zTrade / 4)) : null;
  ...
  return {
    ...
    multiSignal: {
      ...
      buyBurstZScore: zScoreAll.available ? zScoreAll.peakEventZ : null,
      buyBurstNorm01: Math.round(buyBurstNorm * 1000) / 1000,
      zScores: zScoreAll,
      zScoresTrade: zScoreTrade,
    ...
    composite: {
      ...
      formulaNote:
        "Weighted blend: fee concentration, activity burst z-score vs Turso baseline when present else in-window fallback (all events; trade-like swap/SPL z in-window only), funding hub, window first-activity-in-peak, amount uniformity, chain-backed cohort youth (wallet_first_seen), plus detector boosts — heuristic, not court-grade.",
    },
  };
}
