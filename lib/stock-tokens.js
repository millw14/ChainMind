 ...
 *
 * HOLDER COUNT IS STILL HERE, one rung down, because it is the only figure a
 ...
 * equities where no pool is ever read, holders is what is left.
 *
...
export async function withQuoteDepth(candidates, query, probe) {
  ...
  // way here would be ranked as though it were exact.
  return { candidates: applyDepths(candidates, survey.depths, survey.lowerBounds), survey };
}
 ...
 * back to explorer search — but `official` is decided by the DEPLOYER on every
 * branch, never by which branch answered, so a genuine equity that only the search
 * fallback could find is still labelled official. Callers must not present a
 * verified and an unverified contract the same way, and this function must not
 * mislabel one because of how it was reached.
 *
...
export async function resolveSymbol(query, options = {}) {
  ...
  // Fast path: an exact ticker or company name we already have a verified
  // address for. Fetching that one token costs a single request, where the
  // page walk below costs ~7.8s — and the verdict is not in question, since a
  // snapshot address is canonical by definition. The impostor search still
  // runs, because naming the fakes is the point of this module.
  const snap = snapshotMatch(q.raw);
  if (snap) {
    const [live, candidates] = await Promise.all([
      safeToken(snap.address),
      searchCandidates(snap.symbol),
    ]);
    // Fall through to the slow path only if the token itself is unreadable;
    // answering from the snapshot alone would mean quoting no live figures.
    if (live) {
      return {
        ok: true,
        query: q.raw,
        match: { ...live, symbol: live.symbol ?? snap.symbol, company: live.company ?? snap.company },
        official: true,
  ...
  if (match) {
    ...
    return {
      ...
      impostorsRead: Array.isArray(candidates),
      // Same reasoning as the snapshot path: the match came out of the official
      // list, so the deployer decides and depth cannot.
      collision: null,
      ...(isOfficial
        ? {}
        : {
  ...
  // contract rather than of the contract — and issuerRank now routes a canonical
  // equity into this branch deterministically: during an indexer brownout
  // listStockTokens() comes back empty, pickBestMatch over it finds nothing, and
  // the real NVDA is then selected correctly by the search fallback and labelled
  // "not an official Robinhood tokenized equity". The deployer is the root of
  // trust; whichever branch found the contract cannot change what its deployer is.
  // The snapshot answers offline and free, and verifiedByIssuer is one indexer call
  // that fails closed, run concurrently with the search rather than after it.
