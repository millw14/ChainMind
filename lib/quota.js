...
export async function consumeQuestion({
  ...
  if (state === QUOTA_STATE.UNLIMITED) {
    return report({
      allowed: true,
      unlimited: true,
      state,
...
export function quotaMessage(quota, entitlement = null, now = Date.now()) {
  ...
  if (quota.state === QUOTA_STATE.ANONYMOUS) {
    return `${standing} ${holdingLine(entitlement)} Connecting only asks you to SIGN a message — it is not a transaction, it cannot move funds, grant an allowance or spend gas.`;
  }
