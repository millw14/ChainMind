...
- fundingGraph with explicit shared_funders and funding tx signatures: weight 0.7–0.9.
- walletEvidence.shared_funders with ≥1 on-chain signature: weight up to 0.85.
- priorVerdicts with 2+ prior escalations on same scope: add recurrence signal weight 0.6.
...
7. limiting_factors must list what is concretely absent (e.g. "fundingGraph not attached — shared funder unverifiable").
...
export function buildGroqBriefUserContent(evidenceNarrative, evidenceJson, focus) {
  ...
  try {
    ...
    hints.push(
      `fundingGraph = ${hasFundingGraph ? "ATTACHED — check shared_funders" : "NOT ATTACHED — cannot verify funder links"}`,
    );
