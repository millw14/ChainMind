...
import { listStockTokens, resolveSymbol } from "./stock-tokens.js";
...
export async function gatherEvidence(target) {
  ...
  if (kind === "symbol") return await symbolEvidence(value);
 ...
 */
async function symbolEvidence(symbol) {
  const resolvedRes = await attempt(() => resolveSymbol(symbol));
  ...
  if (!match?.address) {
    ...
    const listRes = await attempt(() => listStockTokens());
  ...
  const addrRes = await attempt(() => getAddress(address));
  if (!addrRes.ok || !addrRes.data) return lookupFailure("token", address, addrRes.status, "Token");

  const base = await tokenEvidence(address, addrRes.data);
