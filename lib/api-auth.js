...
export function isSameOriginBrowser(request) {
  ...
  // Compare against the host the request actually arrived on — browsers that omit
  // Fetch Metadata (older Safari) or deployments on a preview/alternate domain are
  // still genuinely same-origin, and must not be locked out of their own dashboard.
  const forwarded = request.headers.get("x-forwarded-host");
  const selfHost = (forwarded || request.headers.get("host") || "").split(",")[0].trim();
  if (selfHost && originHost === selfHost) return true;
  // Configured public origin, for setups that rewrite Host between proxy hops.
...
/**
 * Bearer header for server-to-server self-fetches (cron sweeps hitting our own
 * /api/score etc.) — those carry no Sec-Fetch-Site/Origin, so without this they
 * would be treated as anonymous public traffic and hit the cold-scope budget.
 * @returns {Record<string, string>}
 */
export function internalAuthHeaders() {
  const secret = process.env.CHAINMIND_OPERATOR_SECRET?.trim() || process.env.CRON_SECRET?.trim();
