...
export function summarizeFundingGraphFromEdges(topPayers, rows) {
  ...
  for (const r of rows) {
    ...
    // Infra accounts (DEX routers, vaults, core programs) move tokens to many wallets
    // as normal operation — they are not "shared funders." Excluding them stops a
    // single router/vault from looking like a sybil funding hub.
    if (!payersSet.has(rec) || !fund || INFRA_ACCOUNTS.has(fund)) continue;
