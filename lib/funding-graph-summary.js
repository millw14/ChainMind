...
export function summarizeFundingGraphFromEdges(topPayers, rows) {
  ...
  if (!Array.isArray(rows) || rows.length === 0) {
    return {
      ...
      note:
        "No inbound token_transfer / fee_payer_cosigner / mint_to edges into these payers in this lookback — graph backfill may be incomplete.",
    };
  }
  ...
  for (const r of rows) {
    ...
    if (!payersSet.has(rec) || !fund) continue;
