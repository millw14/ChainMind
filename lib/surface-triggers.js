...
function ruleIngestEventSpike(score, opt) {
  ...
  const b = buckets[idx];
  const startIso = b?.startSec != null ? new Date(Number(b.startSec) * 1000).toISOString() : "—";
  return {
    ruleId: "ingest_event_spike",
    severity: max >= opt.eventSpikeMult * mean * 1.5 ? "high" : "medium",
    title: "Activity burst vs ingest baseline",
    detail: `In one ${score.windowMinutes ?? "?"}m bucket starting ~${startIso}, event count ${max} exceeds ~${opt.eventSpikeMult}× the per-bucket average (${mean.toFixed(1)}). Proxy for “volume spike” until DEX volume is wired.`,
...
function ruleFundingHub(score, opt) {
  ...
  for (const row of fg.sharedInboundFunders) {
    ...
    out.push({
      ...
      detail: `One inbound funder touches ${n} co-active fee payers in the scoring window (graph slice). Threshold ≥ ${opt.minFundedPayers}.`,
...
function ruleDensePayerBurst(score, opt) {
  ...
  return {
    ...
    detail: `${peak} distinct fee payers in a single ${score.windowMinutes ?? "?"}m bucket vs configured threshold ${opt.densePayerMin}.`,
    entities: typeof score.scope === "string" ? [score.scope] : [],
  };
}
