...
export function buildGroqEvidenceBlockText(userEvidence, opts = {}) {
  ...
  lines.push(`## Z-SCORE ANALYSIS`);
  lines.push(`Detector focus: Is recent activity statistically abnormal vs baseline?`);
  if (ev.zScores?.available) {
    lines.push(`Baseline source: ${ev.zScores.baselineSource}`);
    lines.push(`Peak event z-score: ${ev.zScores.peakEventZ}`);
    lines.push(`Peak wallet z-score: ${ev.zScores.peakWalletZ}`);
    lines.push(`Event spike detected: ${ev.zScores.eventSpikeDetected}`);
    lines.push(`Wallet spike detected: ${ev.zScores.walletSpikeDetected}`);
    lines.push(`Sustained acceleration: ${ev.zScores.accelerating}`);
    if (ev.zScores.baselineSource?.includes("fallback")) {
      lines.push(`⚠ In-window fallback — run baseline:update to improve accuracy`);
    }
  } else {
    lines.push(`Z-scores: unavailable (${ev.zScores?.reason ?? "unknown"})`);
  }
  lines.push(``);

  lines.push(`## PRE-POSITIONING SIGNAL`);
  lines.push(`Detector focus: Do z-scores + coactivity converge before public visibility?`);
  if (ev.prePosition) {
    lines.push(`Detected: ${ev.prePosition.detected}`);
    lines.push(`Confidence: ${ev.prePosition.confidence}`);
    lines.push(`Verdict: ${ev.prePosition.verdict}`);
    for (const s of ev.prePosition.signals ?? []) {
      lines.push(`  [${s.type} w=${s.weight}] ${s.detail}`);
    }
    if (ev.prePosition.baselineWarning) {
      lines.push(`⚠ ${ev.prePosition.baselineWarning}`);
    }
  } else {
