import { tursoFetchInboundFundingEdges, tursoFetchInboundFundingEdgesGlobal } from "./turso.js";
...
export async function expandFundingTreeInbound(
  ...
  for (let depth = 1; depth <= maxDepth && nodeMap.size < maxNodes && frontier.length > 0; depth++) {
    ...
    // Use global cross-scope query for deeper hops — scope filter too restrictive for multi-hop
    const rows =
      depth === 1
        ? await tursoFetchInboundFundingEdges(client, scope, slice, sinceUnix, perQueryLimit)
        : await tursoFetchInboundFundingEdgesGlobal(client, slice, sinceUnix, perQueryLimit);
