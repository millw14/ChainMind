...
export async function buildTursoScoreBundle(client, p, opts = {}) {
  ...
  // Baseline lookup depends only on scope+window, so overlap it with the score-rows
  // fetch instead of running it last (each Turso round-trip carries real latency).
  ...
  if (
    ...
    // These all depend only on the wallet-node list, so fetch them in one parallel
    // batch rather than sequential round-trips (was: funding → transfers∥peers → ages).
    const [edgeRows, trows, prow, ageRows] = await Promise.all([
      payers.length > 0
  ...
  // Started in parallel with the score-rows fetch above.
