...
import { NATIVE_SOL_TRANSFER, TOKEN_2022_PROGRAM, TOKEN_PROGRAM } from "./programs.js";
...
export function extractTxGraph(parsed, slot) {
  ...
  function processIx(ix) {
    ...
    let xferAdded = handleJsonSpl(ix, transferIdx, transfers, edges, ownerOfAta);
  ...
  // Write inbound native_transfer edges to feePayer
  // so fee payers appear as to_address and fundingGraph can attach.
  // Uses balance deltas from parsed.meta — no new parameters needed.
  if (feePayer) {
    ...
    if (feePayerIdx >= 0) {
      const feePayerGain =
        (postBalances[feePayerIdx] ?? 0) - (preBalances[feePayerIdx] ?? 0);

      if (feePayerGain > 0) {
        // Find the wallet that lost SOL and is most likely the funder
        for (let i = 0; i < keys.length; i++) {
          ...
          if (loss > 0 && keys[i]) {
            ...
            break; // one inbound funding edge per fee payer per tx
          }
        }
      }
    }
