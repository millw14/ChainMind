...
import { CLARIFICATION_TOOL, TOOL_SCHEMAS, dispatchTool } from "./ask-tools.js";
...
export async function runToolLoop({
  ...
  for (;;) {
    const offerTools = rounds < maxRounds && !terminal;
    ...
    if (handBackAfterTools && (terminal || rounds >= maxRounds || collected.every((entry) => entry.ok))) {
      const summary = summarize(performed, collected);
      return {
        ok: true,
        answered: false,
