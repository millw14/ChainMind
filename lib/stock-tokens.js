 ...
 *
 * @param {string} query
 * @returns {Promise<{ ok: boolean, query: string, match: object|null, official: boolean, impostors: Array<object>|null, impostorsRead: boolean, reason?: string }>}
 */
export async function resolveSymbol(query) {
  const q = normalizeQuery(query);
  // No scan was attempted on a miss, so `impostors` is unknown rather than none.
  const miss = { ok: false, query: q.raw, match: null, official: false, impostors: null, impostorsRead: false };
  ...
  const symbolCandidates =
    !best.symbol || sameSymbol(best.symbol, q.raw) ? candidates : await searchCandidates(best.symbol);
