...
export const THINKING_PHRASES = Object.freeze({
  ...
  // The profile runs three legs at once and takes the longest of any lookup here,
  // so its pool is the widest — a status row that repeats itself for fifteen
  // seconds reads as a hang. Every phrase names chain work, because that is all
  // this lookup does: no phrase may hint at a site being opened.
  [PHRASE_STEPS.PROFILE]: Object.freeze([
    "reading the deployment record",
    "asking what deployed it",
    "dating the contract",
    "sizing the pool behind it",
    "walking the holder base",
    "checking who arrived together",
    "reading the launch calldata",
  ...
  // The three SWAP lookups. They must not sound like the TRANSFERS pool: a transfer
  // walk says something moved, and these read the record of somebody buying or
  // selling — which is the distinction the whole swap layer exists to make, so the
  // status row is the first place a reader meets it. TRADES and WHALES are the pair
  // most easily confused, so nothing here says "size" or "big prints".
  [PHRASE_STEPS.TRADES]: Object.freeze([
    "reading the swaps",
    "sorting buys from sells",
    "seeing who's on each side",
    "checking who's been selling",
    "pulling the last fills",
  ]),
  // A volume-quality read is the same window looked at differently, so its pool
  // names the SHAPE of the flow rather than the flow. No phrase may hint at a
  // verdict: nothing here is deciding whether anything is wash traded, because
  // that is not a thing this lookup can decide.
  [PHRASE_STEPS.VOLUME_QUALITY]: Object.freeze([
    "counting the counterparties",
    "seeing how two-sided it is",
    "measuring how concentrated it is",
    "looking for round trips",
    "checking who's on both sides",
  ]),
  // One transaction, so this is the shortest wait in the catalogue — two chain reads
  // and a filtered log query. The phrases name the parts of a swap a reader asks
  // about: what went in, what came out, and what it cost.
  [PHRASE_STEPS.SWAP]: Object.freeze([
    "unpacking the swap",
    "reading the fee that was charged",
    "checking what went in and out",
    "finding the pool it hit",
    "measuring how far the price moved",
  ]),
...
const TOOL_STEP = Object.freeze({
  ...
  project_profile: PHRASE_STEPS.PROFILE,
  ...
  recent_trades: PHRASE_STEPS.TRADES,
  real_volume: PHRASE_STEPS.VOLUME_QUALITY,
  swap_detail: PHRASE_STEPS.SWAP,
...
export function progressLabel(step, subject) {
  ...
  switch (key) {
    ...
    case PHRASE_STEPS.TRADES:
      return `reading ${s} swaps`;
    case PHRASE_STEPS.VOLUME_QUALITY:
      return `sizing up ${s} volume`;
    // The subject is the transaction hash, already shortened by toolSubject.
    case PHRASE_STEPS.SWAP:
      return `unpacking ${s}`;
