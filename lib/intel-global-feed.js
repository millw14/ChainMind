...
/**
 * Cross-mint pair rows for a compute window (null = any window).
 * @param {import("@libsql/client").Client} client
 */
async function fetchCrossMintPairs(client, lookbackHours, limit) {
  const where = lookbackHours != null ? "WHERE lookback_hours = ?" : "";
  const args = lookbackHours != null ? [lookbackHours, limit] : [limit];
  const res = await client.execute({
    sql: `SELECT scope_a, scope_b, COUNT(*) AS shared_wallets, AVG(pair_score) AS avg_pair_score,
                 MAX(computed_at) AS computed_at
          FROM intel_cross_mint_pair ${where}
...
export async function buildGlobalIntelFeed(client, opts = {}) {
  ...
  // Prefer the requested window, but fall back to the most-recently-computed window when
  // it's empty — so the feed isn't blank just because the dashboard's lookback (e.g. 24h)
  // doesn't match the window the cron computed at (e.g. 720h). Exact-match-only was hiding
  // real pairs whenever the two didn't line up.
  let pairWindowHours = lookbackHours;
  try {
    let pairRows = await fetchCrossMintPairs(client, lookbackHours, limit);
    if (pairRows.length === 0 && lookbackHours != null) {
      const latest = await client.execute({
        sql: `SELECT lookback_hours FROM intel_cross_mint_pair ORDER BY computed_at DESC LIMIT 1`,
      });
      const lh = latest.rows[0]?.lookback_hours;
      if (lh != null && Number(lh) !== lookbackHours) {
        pairWindowHours = Number(lh);
        pairRows = await fetchCrossMintPairs(client, pairWindowHours, limit);
      }
    }
