...
export function buildGroqBriefUserContent(evidenceNarrative, evidenceJson, focus) {
  ...
  try {
    ...
    // wallets_total reflects the real count when the client truncated the array to
    // keep the request body small (see trimWalletEvidence) — prefer it for the count.
    const walletCount = hasWalletEvidence
      ? parsed.walletEvidence.wallets_total ?? parsed.walletEvidence.wallets.length
      : 0;
    const walletSample = hasWalletEvidence ? parsed.walletEvidence.wallets.length : 0;
    hints.push(
      `walletEvidence = ${
        hasWalletEvidence
          ? `${walletCount} wallets present${walletCount > walletSample ? ` (${walletSample} sampled in payload)` : ""}`
          : "absent"
      }`,
    );
