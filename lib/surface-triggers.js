...
export function evaluateSurfaceTriggers(input, env) {
  ...
  // Known entities (stablecoins, wrapped SOL, CEX/router/launchpad wallets) have
  // naturally high fee-payer/activity volume — they are not coordination targets, so
  // they should never produce surface hits (USDC was flooding the feed with false flags).
  const scope = typeof score?.scope === "string" ? score.scope.trim() : "";
  if (scope && isKnownEntity(scope)) {
