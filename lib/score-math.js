...
export function buildTimelineBucketsFromRows(rows, windowMinutes, eventTypes = null) {
  ...
  // Dense grid: gap-fill zero-count buckets between the oldest and newest keys so
  // downstream mean/std/z-scores are computed over time, not over active buckets
  // only. The 320-bucket cap is kept by trimming from the oldest (newest end wins).
  const bucketKeys = [...buckets.keys()];
  const maxKey = Math.max(...bucketKeys);
  const minKey = Math.max(Math.min(...bucketKeys), maxKey - 319);

  /** @type {{ startSec: number, endSec: number, walletCount: number, eventCount: number }[]} */
  const timelineBuckets = [];
  for (let b = minKey; b <= maxKey; b++) {
    timelineBuckets.push({
      startSec: b * bucketSec,
      endSec: (b + 1) * bucketSec,
      walletCount: buckets.get(b)?.size ?? 0,
      eventCount: bucketEventCounts.get(b) ?? 0,
    });
  }
...
export function computeCoactivityScoreFromRows(rows, scope, windowMinutes, lastHours) {
  ...
  for (const r of rows) {
    const t = r.block_time;
    const b = Math.floor(t / bucketSec);
    // Skip infra/known-entity fee payers (routers, vaults, CEX/market-maker wallets) —
    // they aren't coordinating wallets, so they shouldn't inflate distinct-payer
    // counts or the wallet graph.
    const isInfraPayer = r.fee_payer && isKnownEntity(r.fee_payer);
    if (!buckets.has(b)) buckets.set(b, new Set());
    if (!isInfraPayer) buckets.get(b).add(r.fee_payer);
    bucketEventCounts.set(b, (bucketEventCounts.get(b) ?? 0) + 1);

    if (r.fee_payer && !isInfraPayer) {
