...
function hop(url, { lookupFn, timeoutMs, maxBytes, userAgent, signalDeadline }) {
  return new Promise((resolve) => {
    ...
    req.on("error", (e) => {
      ...
      const label = code === "ENOTFOUND" ? `${url.host} does not resolve` : code === "ETIMEDOUT" || e?.message === "timeout" ? `${url.host} did not answer in time` : `${url.host} could not be reached (${code || String(e?.message ?? e).slice(0, 80)})`;
      done({
        ok: false,
        code: code === "ENOTFOUND" ? "dns" : "network",
        refusal: `${label}. That is a failure to READ the site, not a finding that the site is down or does not exist.`,
      });
    });
