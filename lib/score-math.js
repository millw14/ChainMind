...
export function computeCoactivityScoreFromRows(rows, scope, windowMinutes, lastHours) {
  ...
  for (const r of rows) {
    const t = r.block_time;
    const b = Math.floor(t / bucketSec);
    // Skip infra fee payers (routers/vaults/programs) — they aren't coordinating
    // wallets, so they shouldn't inflate distinct-payer counts or the wallet graph.
    const isInfraPayer = r.fee_payer && INFRA_ACCOUNTS.has(r.fee_payer);
    if (!buckets.has(b)) buckets.set(b, new Set());
