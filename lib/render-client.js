 ...
 *
 * IT IS OUR OWN INFRASTRUCTURE AND IT IS STILL CHECKED — a typo pointing this at
 * `http://169.254.169.254/` would turn the app's own configuration into an SSRF. But it
 * must NOT be checked with validateUrl, which is the policy for an INVESTIGATED TARGET,
 * and the two have opposite requirements. Measured: validateUrl refuses
 * `http://127.0.0.1:8391/render` and `http://render.railway.internal:8080/render` on
 * code `port`, while only the public `https://….up.railway.app/render` passes. A private
 * hostname on a non-default port is the NORMAL shape of an internal service and the whole
 * point of Railway's private networking — refusing it forces this traffic onto the public
 * internet, or blocks the deployment outright.
 *
 * So: shape and scheme are required, the cloud metadata endpoint stays refused because a
 * render service is never legitimately there, and ports plus private and loopback hosts
 * are allowed because that is where an internal service actually lives.
 *
 * Returns a VERDICT, not a bare value. A configured-but-rejected URL is a different fact
 * from an absent one, and collapsing them reported a rejection as "not configured" — an
 * operator debugging that would be told the opposite of what happened.
 */
function serviceBase(env) {
  ...
  let parsed = null;
  try {
    parsed = new URL(`${raw}/render`);
  } catch {
    return { base: null, problem: `RENDER_SERVICE_URL is not a URL: ${raw.slice(0, 80)}` };
  }
  if (parsed.protocol !== "http:" && parsed.protocol !== "https:") {
    return { base: null, problem: `RENDER_SERVICE_URL must be http or https, not ${parsed.protocol}` };
  }
  const host = parsed.hostname.toLowerCase().replace(/^\[|\]$/g, "");
  // The one address a render service is never legitimately at. Everything else an
  // operator points this at is their own deliberate choice about their own network.
  if (host === "169.254.169.254" || host === "metadata.google.internal" || host === "metadata") {
    return { base: null, problem: "RENDER_SERVICE_URL points at the cloud metadata endpoint, which is never a render service" };
  }
...
export async function renderPage(url, options = {}) {
  ...
  // A REJECTED SETTING IS NOT AN ABSENT ONE. This used to answer "not_configured" for a
  // RENDER_SERVICE_URL that was set and refused, which tells an operator to add the very
  // variable they already added. Same page-level consequence, entirely different fix.
  if (problem) {
    return {
      ...shell,
      status: "misconfigured",
      reading: `NO BROWSER RENDER WAS ATTEMPTED because the render service is misconfigured: ${problem}. That is a fault in THIS DEPLOYMENT's settings, not a finding about the site — the page below was read as the shell its server sends. See services/render/README.md.`,
    };
  }
  if (!base || !secret) {
    const missing = [!base ? "RENDER_SERVICE_URL" : null, !secret ? "RENDER_SHARED_SECRET" : null].filter(Boolean).join(" and ");
    return {
      ...shell,
      status: "not_configured",
      reading: `NO BROWSER RENDER WAS ATTEMPTED because this deployment has no render service configured (${missing} not set). That is a fact about THIS DEPLOYMENT and nothing at all about the site: a client-rendered page will therefore have been read as the shell its server sends, which on a JavaScript-built site can be a handful of characters. See services/render/README.md.`,
    };
  }
