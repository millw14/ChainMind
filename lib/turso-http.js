...
export async function tursoHttpFetchPendingScanQueue(limit = 10) {
  ...
  // 'retired' rows (known entities) are terminal and never served. Least-recently-picked
  // first, so rotation eventually serves every queued address.
  ...
  try {
    const rows = await tursoExecute(client,
      `SELECT address, added_at, note FROM scan_queue
       ...
       ORDER BY COALESCE(last_picked_at, 0) ASC, added_at ASC LIMIT ?`,
      [staleSeconds, limit]
    );
...
/** Terminal state for addresses that will never become scopes (stablecoins/infra). */
export async function tursoHttpMarkScanQueueRetired(address) {
  const client = getTursoHttp();
  if (!client) return;
  try {
    await tursoExecute(client,
    ...
    console.error('[scan-queue] mark-retired failed:', e?.message ?? e);
  }
}
