...
/**
 * IS A V3 ABSENCE AN ABSENCE OF A MARKET — the question this file used to skip, and the
 * ranking half of the defect that made PIPECAT read as having no market at all.
 *
 * WHAT WAS WRONG. `no_pool` was recorded as a MEASURED depth of zero, and it deserved to
 * be: a token nothing trades really does have nothing realisable behind it, and that zero
 * is what lets a field of dust be reported as shallow rather than as unknown. But the
 * premise only holds if the sweep that produced it looked EVERYWHERE, and it looked only
 * at Uniswap v3. Measured on chain 4663, PIPECAT 0x31ba…c6cc has no v3 pool and eight v4
 * pools, one of them carrying $45.90 of realisable depth and 114 of the 116 swaps its
 * pools saw in 300,000 blocks. Recorded as a measured zero, that is not a missing figure —
 * it is a WRONG figure, asserted, and it is the strongest kind of statement this file
 * makes: a zero cannot be beaten, so it settles a ticker collision against the contract
 * that actually has the market.
 *
 * WHAT IT IS NOW. A v3 absence is a measured zero only when the v4 read ALSO came back as
 * a stated absence. Anything else — v4 pools found, or a v4 read that could not be made —
 * leaves the contract UNMEASURED, which loses every comparison rather than winning one and
 * is reported as unknown rather than as shallow.
 *
 * A V4 FIGURE IS DELIBERATELY NOT SUBSTITUTED IN HERE. When v4 prices the token,
 * lib/dex-price.js has already put the v4 band depth in `quoteLiquidityUsd` with
 * `source: "uniswap_v4"` beside it, so quoteDepth finds it by the ordinary path and this
 * function is never reached. What this function refuses to do is manufacture a figure for
 * a token whose v4 pools were found but NOT priced — that depth is unknown, and unknown
 * must not be dressed as either a zero or a win.
 */
function isMeasuredAbsence(market) {
  if (!MEASURED_ABSENCE_REASONS.has(market?.reason)) return false;
  const v4 = market?.v4 ?? null;
  // No v4 read attached at all is the pre-v4 shape and the honest reading of it is
  // "nobody asked about v4" — so it cannot be a swept absence either.
  if (!v4) return false;
  return v4.status === "none";
}
...
export async function probeQuoteDepth(address, options = {}) {
  ...
  try {
    const res = await withDeadline(
      withIndexerCache(
        ...
        async () => {
          ...
          if (depth !== null) {
            // THE VENUE TRAVELS WITH THE FIGURE, because the figure alone no longer says
            // which instrument produced it: lib/dex-price.js puts a v4 band depth in this
            // same field when v4 is what priced the token. Both are the same integral over
            // the same band, which is exactly why the label cannot be inferred from the
            // number and has to be carried beside it.
            return {
              quoteLiquidityUsd: depth,
              depthIsLowerBound: depthIsLowerBound(market),
              depthSource: market?.source === "uniswap_v4" ? "uniswap_v4" : "uniswap_v3",
            };
          }
          ...
          if (isMeasuredAbsence(market)) {
            // A zero established on BOTH venues belongs to neither of them, and labelling it
            // "uniswap_v3" would credit one instrument with a finding the pair of them made.
            return { quoteLiquidityUsd: 0, depthIsLowerBound: false, depthSource: "both_venues" };
          }
    ...
    return depthUsd === null
      ? null
      : { depthUsd, lowerBound: res?.depthIsLowerBound === true, source: res?.depthSource ?? null };
  } catch {
    return null;
  }
}
...
export async function measureDepths(tier, options = {}) {
  ...
  // WHICH INSTRUMENT MEASURED EACH FIGURE. Its own map, for the same reason lowerBounds is
  // one: a label kept beside the number rather than folded into it cannot be read without
  // the reader having had the chance to see it, and a v3 depth and a v4 depth must never
  // reach an answer as one unlabelled quantity.
    ...
    shortlist.map(async (c) => {
      try {
        ...
        if (typeof res === "object") {
          ...
          return depthUsd === null
            ? null
            : { depthUsd, lowerBound: res.lowerBound === true, source: res.source ?? null };
        }
        const depthUsd = finiteOrNull(res);
        // A bare number carries no venue, and inventing one for it would be a label nobody
        // measured — null reads as "not recorded", which is what it is.
        return depthUsd === null ? null : { depthUsd, lowerBound: false, source: null };
      } catch {
        return null;
      }
    }),
  ...
  shortlist.forEach((c, i) => {
    ...
    if (results[i].source) sources.set(c.address, results[i].source);
  });
 ...
 */
export function applyDepths(candidates, depths, lowerBounds = null, sources = null) {
  ...
  // Optional, and last, so every existing caller keeps working unchanged. An absent map
  // leaves the label NULL, which reads as "not recorded" rather than as a claim that the
  // figure is v3's — the venue is a measurement like any other and is not defaulted.
  const venue = sources instanceof Map ? sources : new Map();
  return candidates.map((c) =>
    c && depths.has(c.address)
      ? {
          ...c,
          quoteLiquidityUsd: depths.get(c.address),
          depthIsLowerBound: bounded.has(c.address),
          depthSource: venue.get(c.address) ?? null,
        }
      : c,
  );
}
...
export function dominanceVerdict(rows, survey = {}) {
  ...
  return {
    ...
    measuredRows: measured.map((c) => ({
      ...
      // WHICH VENUE MEASURED IT. Null means the label was not recorded, which is a different
      // thing from "v3" and must not be rendered as it — the layer that writes the sentence
      // reports the depth source as unknown rather than guessing at one.
      depthSource: c.depthSource ?? null,
    })),
