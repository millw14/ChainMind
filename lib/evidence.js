...
// Timeline spine. Tuned against real load: on the hottest scope (BONK, ~45k edges) a
// 25k cap still took ~75s — the rows shipped over libSQL HTTP dominate. 12k keeps the
// most-recent activity (more than enough to rank thousands of wallets) and brings the
// worst case comfortably under the request budget.
const MAX_TIMELINE_EVENTS = 12000; // spine of the case file; keeps most-recent N
const MAX_TIMELINE_TRANSFERS = 12000; // transfer legs joined onto the timeline
// Resolve funding only for the most active wallets — the cores of any coordinated
// cluster. Low-activity wallets rarely anchor shared-funder signals, and capping this
// bounds the number of chunked round-trips below.
const MAX_FUNDING_WALLETS = 1500;
...
export async function buildWalletTable(db, scopeAddress, timeline) {
  ...
  // Resolve first inbound funder for the most active wallets (cluster cores). Chunk the
  // IN() list so a busy scope's wallet set can't trip the bind-parameter ceiling (which
  // previously failed the whole query and dropped all funding links).
  const addresses = [...walletMap.values()]
    .sort((a, b) => b.tx_count - a.tx_count)
    .slice(0, MAX_FUNDING_WALLETS)
    .map((w) => w.address);
