 ...
 *
 * Native SOL transfers are not yet represented as edges — funding-like flows use
 * `token_transfer`, `mint_to`, and `fee_payer_cosigner` from the ingest pipeline.
 *
 * @module graph
 */
...
export function fundingTree(db, rootAddress, opts) {
  ...
  let sql = `
    ...
    WHERE e.scope_address = ?
      AND e.from_address = ?
      AND e.edge_type IN ('token_transfer', 'fee_payer_cosigner', 'mint_to')
  `;
  /** @type {(string|number|null)[]} */
...
/**
 * Incoming SPL-style funders (token_transfer authority edges point here as to_wallet).
 *
 * @param {import("better-sqlite3").Database} db
 */
function incomingFundersForWallet(db, scopeAddress, wallet, sinceUnix, untilUnix) {
  let sql = `
    ...
    WHERE e.scope_address = ?
      AND e.to_address = ?
      AND e.edge_type = 'token_transfer'
  `;
  /** @type {(string|number|null)[]} */
  const params = [scopeAddress, wallet];
