 ...
 *
 * `batch` packs eth_calls issued in the same tick into ONE JSON-RPC request. It
 * is off by default because it changes the shape of what goes over the wire and
 * not every endpoint accepts an array body — this one does, verified live against
 * chain 4663: a two-call batch returns a two-element array. It is worth asking for
 * wherever a single lookup fans out into dozens of independent reads, which is
 * exactly what the Uniswap v3 pool sweep does.
 *
 * @param {{ timeout?: number, retryCount?: number, batch?: boolean|object }} [options]
 */
export function getPublicClient(options = {}) {
  ...
  if (options.batch != null) transport.batch = options.batch;
