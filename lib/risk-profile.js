...
export function deriveRiskProfile(score) {
  ...
  if (compRaw != null && Number.isFinite(Number(compRaw))) {
    ...
    return {
      ...
      peakPayers: peak,
      model: "v2_composite",
      blurb: `Composite detection index ${score0_100}/100 (fee concentration, burst z-scores, funding hubs, timing/uniformity, chain-backed payer youth, named detectors); legacy peak ${peak} distinct fee payers in one ${score.windowMinutes}-minute slice.`,
    };
  }
