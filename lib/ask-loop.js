...
/** The leading name of a JSON-object tool call, readable even when the rest was cut. */
const JSON_OBJECT_CALL_NAME_RE = /^\s*\{\s*"name"\s*:\s*"([a-z_][a-z0-9_]*)"/i;

/**
 * A tool call in the shape gpt-oss emits: one JSON object, `name` and `arguments`.
 *
 * Returns the call when the text is that shape and names a tool we have, else
 * null. When the object parses, `arguments` is taken as given — nested object or
 * string, either way it is handed to toolCallFrom as JSON text. When it does not
 * parse, the name is read off the front and the arguments are empty: a truncated
 * generation still says which tool was meant, and that is what routing needs.
 */
function jsonObjectToolCall(text, names, index) {
  const m = JSON_OBJECT_CALL_NAME_RE.exec(text);
  if (!m || !names.includes(m[1])) return null;
  let rawArgs = "{}";
  try {
    const parsed = JSON.parse(text);
    const a = parsed?.arguments;
    if (a && typeof a === "object") rawArgs = JSON.stringify(a);
    else if (typeof a === "string" && a.trim()) rawArgs = a;
  } catch {
    // Cut mid-object. The name above is the whole recoverable content.
  }
  const call = toolCallFrom(m[1], rawArgs, index, "refused");
  return call && !call.argsError ? call : null;
}
...
export function recoverRefusedToolCalls(detail, validNames = TOOL_NAMES) {
  ...
  for (const text of candidates) {
    /*
     * THE THIRD SHAPE, AND THE ONLY ONE gpt-oss PRODUCES. Measured on the first bench
     * after Groq removed the Llama models: every one of the five refusals carried
     * `failed_generation` as a JSON OBJECT — `{"name": "recent_trades", "arguments":
     * {"minutes": 60, …}}`, arguments nested rather than stringified, pretty-printed
     * with newlines — and both scanners below, written for `<function=…>` and for
     * `name({…})`, walked straight past it. `recovered: false` on all five, so a call
     * the model made correctly was scored as a refusal and, in production, would
     * have degraded a routed question to the keyword fallback.
     *
     * Two sub-cases. Whole: parse it and take name and arguments as given — the
     * dispatcher's coercers then do what they always did (a `minutes` of 60 is
     * clamped to the 30 the schema allows, which is exactly why the 400 was not the
     * model choosing wrongly). Truncated mid-object, which is what "Failed to parse
     * tool call arguments as JSON" means: the name is still the first thing in the
     * text, so the tool is recoverable even when its arguments are not, and an
     * empty argument object is what the coercers already turn into a sentence the
     * model can act on.
     */
    const asObject = jsonObjectToolCall(text, names, out.length);
    if (asObject) {
      out.push(asObject);
      break;
    }
