...
export function buildGroqEvidence({ address, score, inspect, risk }) {
  ...
  if (fg?.status === "attached") {
    ...
    if (shared.length > 0) {
      ...
      fundingOverlap = `Funding graph sampled (${fg.edgeRowsSampled ?? 0} edges) — ZERO shared funders found. sharedInboundFunders array is empty. shared-funder signal weight MUST be 0.0. Co-activity: peak ${peak} in one ${timeWindow ?? "bucket"}; ${whole} across ${hrs}h.`;
    }
