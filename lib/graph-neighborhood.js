...
export async function buildWalletNeighborhood(client, address, opts = {}) {
  ...
  return { ...out, scope: scope ?? null, edgeCap, neighborLimit, mode: "wallet" };
}

/**
 * Scope (mint/program) participant graph: the most active wallets in a scope's edges.
 * Used when the queried address is a scope, not a wallet — a mint never appears as an
 * edge endpoint, so its wallet neighborhood is empty. This surfaces "who's active in
 * this token" instead, and each wallet drills into its own (wallet-mode) neighborhood.
 *
 * @param {import("@libsql/client").Client} client
 * @param {string} scope base58 mint/program
 * @param {{ edgeCap?: number, neighborLimit?: number }} [opts]
 */
export async function buildScopeParticipants(client, scope, opts = {}) {
  const edgeCap = Math.min(8000, Math.max(50, Number(opts.edgeCap) || NEIGHBORHOOD_DEFAULTS.edgeCap));
  const neighborLimit = Math.min(500, Math.max(1, Number(opts.neighborLimit) || NEIGHBORHOOD_DEFAULTS.neighborLimit));

  // Scan the most-recent edgeCap edges in this scope, then count BOTH endpoints as
  // participants (from = sent, to = received). Bounded by the LIMIT; indexed on scope.
  const res = await client.execute({
    sql: `SELECT from_address AS f, to_address AS t, edge_type, slot
          FROM edges WHERE scope_address = ? ORDER BY slot DESC LIMIT ?`,
    args: [scope, edgeCap],
  });
  const rows = [];
  for (const e of res.rows) {
    rows.push({ neighbor: e.f, edge_type: e.edge_type, outbound: 1, slot: e.slot });
    rows.push({ neighbor: e.t, edge_type: e.edge_type, outbound: 0, slot: e.slot });
  }
  const out = shapeNeighborhood(scope, rows, { edgeCap: edgeCap * 2, neighborLimit });
  return { ...out, scope, edgeCap, neighborLimit, mode: "scope" };
}
