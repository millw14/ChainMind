 ...
 */
/**
 * RAISED FROM 30 TO 60 SO A BROWSER RENDER CAN FIT IN A CHAT ANSWER.
 *
 * Measured on the live site: a project lookup spent ~20s on chain reads, and
 * lib/render-client.js will not START a render with less than RENDER_CLIENT_LIMITS.MIN_MS
 * (7s) left, allowing up to TIMEOUT_MS (12s) for it. Against a 24s budget that never
 * cleared — the answer came back "THIS PAGE WAS NOT RENDERED THIS TIME … this request did
 * not have that left after the chain reads", which was honest and also meant the render
 * service was effectively unreachable from chat.
 *
 * 60 rather than higher: it is the ceiling a Vercel Hobby plan allows for a Node function,
 * so this number does not silently depend on which plan the deployment is on. Pro allows
 * 300, and nothing here would benefit — a chat answer that takes five minutes is a worse
 * failure than one that admits it ran out of time.
 *
 * THIS IS A CEILING, NOT A TARGET. Ordinary answers land in 5-20s and are unaffected; the
 * budget only decides what the slow path is allowed to attempt before it must degrade.
 */
