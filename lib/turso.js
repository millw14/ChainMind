...
/**
 * Fixed-window rate limit. Returns { allowed, count } for `key` in the current minute.
 * @param {import("@libsql/client").Client} client
 * @param {string} key client identifier (e.g. IP)
 * @param {number} limit max requests per minute
 */
export async function tursoRateLimit(client, key, limit) {
  ...
  try {
    ...
    return { allowed: true, count: 0 }; // fail-open: never block on limiter error
  }
}
