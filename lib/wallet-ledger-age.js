...
export function buildWalletLedgerAge(topPayers, dbRows, nowSec = Math.floor(Date.now() / 1000)) {
  ...
  for (const addr of topPayers) {
    ...
    rows.push({
      ...
      historyCapped: Boolean(Number(r.capped)),
    });
  }
  ...
  return {
    ...
    note:
      status === "not_fetched"
        ...
        : `${withData}/${n} top payers have first signature time from RPC; ${young7d} younger than 7d in this cohort. History may be capped (see historyCapped per row).`,
