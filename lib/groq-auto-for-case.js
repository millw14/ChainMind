...
export async function runGroqBriefForInvestigationCase(p) {
  ...
  // CASE_CREATE_SECRET included so a legacy cases-only setup (no operator/brief
  // secret) can still complete its operator-approved autoGroq self-fetch.
  const briefSecret =
    process.env.CHAINMIND_OPERATOR_SECRET?.trim() ||
    process.env.GROQ_BRIEF_SECRET?.trim() ||
    process.env.CRON_SECRET?.trim() ||
    process.env.CASE_CREATE_SECRET?.trim();
