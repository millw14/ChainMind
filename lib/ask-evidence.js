...
/**
 * `unavailable: [...]` for the evidence block, omitted when nothing failed — plus
 * `budgetNotice` when something was skipped for TIME rather than lost to a failure.
 *
 * Two lists, deliberately, because they are two different facts. `unavailable` names
 * a source that was asked and did not answer; the notice names one that was never
 * asked, because the request would have been killed by the platform before it could
 * be. Both mean "do not read this as zero", and only the second is something the
 * product did to itself — which the reader is entitled to know, since a bound they
 * cannot see is a lie by omission. See lib/request-budget.js.
 */
function gaps(src) {
  const notice = budgetNotice();
  return {
    ...(src.unavailable.length ? { unavailable: [...src.unavailable] } : {}),
    ...(notice ? { budgetNotice: notice } : {}),
  };
}
...
/**
 * The least remaining request time that makes the pool read worth STARTING.
 *
 * From the same measurements as POOL_TIMEOUT_MS: 5.2s for a pool the factory names
 * outright, 6.6-7.9s to establish that a token has none. Below five seconds there is
 * no outcome this read can reach, and starting it anyway produces a timeout — which
 * this file then has to report as `outage`, i.e. "the pool could not be read". That
 * sentence sends a reader looking for an RPC problem that did not happen. Skipping
 * it and saying the clock ran out is the same shortage, told truthfully.
 */
const POOL_MIN_MS = 5_000;
 ...
 */
/**
 * WHICH VENUE A FIGURE CAME FROM, as a label a sentence can use.
 *
 * `source` is set by lib/dex-price.js and is the only thing that decides this — never the
 * shape of the pool identifier, never whether a `fee` looks like a v3 tier. A v4 pool has a
 * 32-byte id where a v3 pool has an address, and inferring the venue from that would be a
 * second, weaker definition of a fact the producer already states.
 */
function venueLabel(source) {
  return source === "uniswap_v4" ? "Uniswap v4" : "Uniswap v3";
}

/**
 * HOW A V4 POOL IS NAMED. It has no address, because every v4 pool on the chain lives
 * inside one PoolManager contract — so the thing to quote is the pool ID, and the singleton
 * beside it, or a reader will go looking for a contract that does not exist.
 */
function poolIdentity(market) {
  const id = market?.pool ?? null;
  if (!id) return "";
  return market?.source === "uniswap_v4" ? `pool ${id} inside the v4 PoolManager singleton` : `pool ${id}`;
}
...
function poolBlock(market) {
  ...
  const source = market.source === "uniswap_v4" ? "uniswap_v4" : "uniswap_v3";
  const venue = venueLabel(source);

  return {
    priced,
    // THE VENUE, ECHOED AND NEVER ASSUMED. This was the literal string "uniswap_v3", which
    // was true while v3 was the only reader and became a mislabel the moment it was not:
    // a v4-priced token would have carried v3's name on v4's figures, which is precisely
    // the unlabelled blend the whole provenance rule exists to forbid.
    source,
    // A V4 POOL HAS NO ADDRESS. `address` stays for the v3 case that every consumer already
    // reads; `poolId` is what a v4 figure is identified by, and `venue` says which to trust.
    address: source === "uniswap_v3" ? (market.pool ?? null) : null,
    poolId: source === "uniswap_v4" ? (market.pool ?? null) : null,
    venue,
    // The hook contract, when the v4 pool names one — a contract that runs INSIDE the swap.
    // Null for v3, which has no such thing, and null for a v4 pool without one.
    hooks: market.hooks ?? null,
    hooked: market.hooked === true,
    tickSpacing: finiteOrNull(market.tickSpacing),
    fee: market.fee ?? null,
    feeTier: tier,
    ...
    // can observe.
    sourceNotice: priced
      ? `This price is derived from the ${venue} ${poolIdentity(market)}${tier ? ` (${tier} fee tier)` : ""} against ${label} — computed from the pool's state at${market.asOfBlock != null ? ` block ${market.asOfBlock}` : " the block the lookup read"}, not quoted by a price feed.${
          finiteOrNull(market.poolCount) > 1
            ? ` This token has ${market.poolCount} ${venue} pools; the figures come from whichever measured the greatest realisable depth, and the others are not shown. Anyone may deploy a pool for a token they do not own, so that selection is a measurement and not a warrant of authenticity.`
            : ""
        }${
          // THE HOOK CLAUSE, ONLY WHEN THERE IS A HOOK, and it qualifies the DEPTH rather
          // than the price. A v4 hook is a contract the singleton calls inside the swap: it
          // can charge a dynamic fee or return a delta of its own, so the committed state
          // says what it says and what a trade actually returns may differ. Measured, the
          // busiest of PIPECAT's eight pools is hooked, so this is the normal case and not
          // an exotic one — and the honest move is to name it, not to discount the figure
          // by an amount nobody can compute.
          market.hooked === true
            ? ` This pool names a HOOK at ${market.hooks} — a contract Uniswap v4 calls inside every swap on it. The price and depth here are read from the pool's committed state, which is a fact about the state and not a promise about what a swap returns: a hook may charge a fee of its own or alter the trade. Say that a hook is attached.`
            : ""
        }${
          // BOTH VENUES, WHEN THERE ARE BOTH. A reader shown only the v3 side of a token
          // that also trades on v4 has been told the truth about one venue and left to
          // assume it is the whole market.
          source !== "uniswap_v3"
            ? ""
            : market.alsoOnUniswapV4 === true
              ? ` This token ALSO has ${finiteOrNull(market.v4?.poolCount) ?? "one or more"} Uniswap v4 pool(s). The figures above are v3's alone and none of them is a blend of the two venues; the v4 side is reported separately under "v4" and must never be added to or averaged with these.`
              : // UNKNOWN IS NOT "NO", and it is said rather than left out. A v4 read that did
                // not answer means these v3 figures may be part of the market rather than all
                // of it, and a reader shown them with no qualifier would reasonably take them
                // for the whole. Only a v4 read that came back a stated absence earns silence.
                market.alsoOnUniswapV4 == null
                ? " Whether this token ALSO trades on Uniswap v4 was not established this call, so these v3 figures may be part of its market rather than all of it. Do not say it trades only on v3."
                : ""
        }${
          // WHY THE PRICE IS V4'S, and the two ways that happens are not the same fact.
          // v3 answering "there is no pool here" is a swept absence; v3 failing to answer
          // is an outage, and claiming the sweep found nothing would be inventing a
          // measurement — the mistake this file exists to prevent, pointed sideways.
          source === "uniswap_v4"
            ? market.v3Reason === "no_pool"
              ? " Uniswap v3 was swept for this token and has no pool for it, which is why the price comes from v4. Do not describe this token as having no market."
              : ` The Uniswap v3 side was NOT settled this call (${market.v3Reason ?? "reason unrecorded"}), so this price is v4's alone and no claim is made about whether a v3 pool also exists. Do not describe this token as having no market, and do not describe it as trading only on v4.`
            : ""
        }`
      : null,
    liquidityNotice: priced
      ? liquidityNotice(quoteLiquidityUsd, liquidityUsd, thin, label, quoteBalanceUsd, {
          ...
          // THE VENUE, BECAUSE ONE CLAUSE OF THAT SENTENCE IS ONLY TRUE OF V3. See the
          // `parked` clause in liquidityNotice: "what the pool holds could not be read" is
          // the right sentence for a v3 balance that failed and a false one for v4, where
          // the quantity does not exist per pool at all.
          source,
        })
      : null,
    ...
    // WHAT THE OTHER VENUE SAID, always, and never folded into the figures above.
    //
    // Carried even when v3 priced the token, because the two most important sentences this
    // block can produce need it: "this token also trades on v4" for a token with both, and
    // "v4 was NOT swept" for a token where v3 found nothing and the v4 read failed — which
    // must not be reported as "no market anywhere".
    v4: market.v4 ?? null,
    // THREE-VALUED, ECHOED, NEVER COERCED. `=== true` here would have turned the producer's
    // honest NULL — "the v4 read did not answer, so whether this token also trades there is
    // unknown" — into `false`, which is the assertion the producer refused to make. Measured
    // live on The Green Bull, whose v4 query failed and which has 48 v4 pools.
    alsoOnUniswapV4: market.alsoOnUniswapV4 ?? null,
    ...
    //
    // IT STAYS A STATEMENT ABOUT THE V3 READ ONLY, deliberately. It was tempting to widen
    // it to "either venue did not settle", since the flag's consumers are asking whether an
    // absence may be stated — but every SENTENCE it selects says the v3 pool could not be
    // read, and firing it for a clean v3 negative beside an unread v4 would send a reader
    // looking for an outage that did not happen. The two facts are reported separately and
    // combined in priceAvailability, where both are in hand.
    ...
    // THE QUESTION THE TWO FLAGS TOGETHER ANSWER: was the market question SETTLED? False
    // means at least one venue was not read, whatever the other one said, so "this token has
    // no market" may not be stated. Computed here rather than at each call site because
    // getting it wrong is the whole defect: v3's `no_pool` is only an absence of a MARKET
    // when v4 also came back a stated absence.
    // NO V4 VERDICT AT ALL IS UNSETTLED, not settled — the same reading lib/depth-rank.js
    // isMeasuredAbsence takes of the same missing field, and for the same reason: nobody
    // asked about v4, so nothing about v4 may be concluded. The two consumers must not
    // disagree about what a missing verdict means, or the same token would be "no market"
    // in one sentence and "unknown" in a ranking on the same page.
    marketSettled:
      priced ||
      (!POOL_UNREAD_REASONS.has(market.reason) &&
        (market.v4?.status === "none" || market.v4?.status === "priced")),
...
function liquidityNotice(quoteLiquidityUsd, liquidityUsd, thin, label, quoteBalanceUsd = null, bands = {}) {
  ...
  // THREE CASES, AND THE THIRD IS NEW BECAUSE V4 HAS NO SUCH QUANTITY.
  //
  // "Could not be read" was the only null branch, and against a v4 pool it is a false
  // sentence: nothing failed. Uniswap v4 keeps every pool in ONE singleton which custodies
  // every pool's tokens together, so there is no per-pool balance to read — balanceOf on
  // the PoolManager is the chain-wide total across every pool holding that token, which is
  // exactly the confusion that made the singleton look like a wallet with a conviction
  // position in two memecoins. Reporting an outage for a figure that does not exist is the
  // mirror of reporting an absence for one we could not read, and this file forbids both.
  const parked =
    bands.source === "uniswap_v4"
      ? " There is no separate “held” figure for a Uniswap v4 pool: the v4 PoolManager is a single contract that custodies EVERY pool's tokens together, so its balance is a chain-wide total across every pool holding this token and is not this pool's holdings. The realisable figure above is measured from this pool's own tick ladder. Do not quote the PoolManager's balance as this pool's liquidity."
      : held === null
 ...
 * @param {boolean} poolSkipped - the pool read was NEVER MADE, because the request
 *   ran short of time. A FIFTH state, and it is not any of the other four. It is not
 *   `not_indexed`, which asserts that neither source has a price and would be a
 *   claim about a pool nobody opened; and it is not the outage above, which sends a
 *   reader looking for a broken RPC. Nothing was measured and nothing failed.
 ...
 */
function priceAvailability(price, marketDataRead, pool = null, poolOutage = false, poolSkipped = false) {
  if (finiteOrNull(price) !== null) {
    return { priceStatus: "indexed", priceStatusReason: null, priceSource: "indexer" };
  }
  if (pool?.priced) {
    const venue = venueLabel(pool.source);
    return {
      priceStatus: "pool_priced",
      // WHICH VENUE, ECHOED FROM THE MEASUREMENT. This was the constant "uniswap_v3", and
      // once a price can come from v4 a constant here is a mislabel that survives all the
      // way into the answer: the model is instructed to name `priceSource`, so a v4 figure
      // would have been introduced to the reader as a v3 one.
      priceSource: pool.source === "uniswap_v4" ? "uniswap_v4" : "uniswap_v3",
      priceStatusReason: `This indexer carries no price feed for this contract, so the price and market cap are derived from its ${venue} pool instead — 24h volume has no pool equivalent and is still absent.${
        pool.source === "uniswap_v4"
          ? " Uniswap v4 keeps every pool in one PoolManager contract, so this pool is named by a 32-byte pool ID rather than by a pool address; there is no separate contract to look up."
          : ""
      }${
        pool.alsoOnUniswapV4 === true && pool.source === "uniswap_v3"
          ? " This token also has Uniswap v4 pools; every figure here is v3's and none of them is a blend of the two."
          : ""
      }`,
    };
  }
  ...
  if (poolSkipped) {
    return {
      ...
      priceSource: null,
      priceStatusReason:
        "No price feed for this contract on the indexer, and its Uniswap v3 pools were NOT READ this time — the lookup ran out of the time it had to answer in before that read could start. So whether this token has a market price here is unknown: nothing was measured and nothing failed. Do not say it has no price, and do not describe this as an outage.",
    };
  }
  /**
   * NEITHER VENUE PRICED IT — AND WHETHER THAT IS AN ABSENCE DEPENDS ON THE V4 READ.
   *
   * THIS IS THE SENTENCE THE DEFECT PRODUCED. It used to end "…and no Uniswap v3 pool
   * against a verified quote asset was found for it either", which is a true statement
   * about v3 that a reader can only take as "this has no market" — and a live session said
   * exactly that about PIPECAT, whose market is a v4 pool holding half its supply. The v3
   * clause was never wrong; it was the whole of what was looked at, presented as the whole
   * of what exists.
   *
   * So the absence is only stated when BOTH venues answered "nothing here", and the three
   * other combinations get their own sentences: v4 unread, v4 not started for time, and v4
   * pools that exist but could not be priced. Each of those is an open question, and none of
   * them may be reported as a token without a market.
   */
  const v4 = pool?.v4 ?? null;
  if (pool && v4 && v4.status !== "none") {
    const shared =
      "No price feed for this contract on the indexer, and no Uniswap v3 pool against a verified quote asset was found for it either.";
    if (v4.status === "found_unpriced") {
      return {
        priceStatus: "unavailable",
        priceSource: null,
        priceStatusReason:
          `${shared} It DOES have ${finiteOrNull(v4.poolCount) ?? "one or more"} Uniswap v4 pool(s), and those could not be turned into a price this call. So this token HAS an on-chain market and what it is worth is unknown — it is emphatically not a token without a market, and not priced at zero. ${v4.detail ?? ""}`.trim(),
      };
    }
    if (v4.status === "skipped") {
      return {
        ...
        priceSource: null,
        priceStatusReason:
          `${shared} Its Uniswap v4 pools were NOT READ this time — the lookup ran out of the time it had to answer in before that read could start. So whether this token has a market is UNKNOWN: nothing was measured and nothing failed. Do not say it has no market, and do not describe this as an outage.`,
      };
    }
    return {
      priceStatus: "unavailable",
      priceSource: null,
      priceStatusReason:
        `${shared} The Uniswap v4 side could not be read, so whether this token has a v4 market is unknown — an outage, not a token without a market. Uniswap v4 keeps every pool in one PoolManager singleton, which is a separate read from the v3 sweep and is the one that failed. ${v4.detail ?? ""}`.trim(),
    };
  }
  return {
    ...
    priceStatusReason:
      `No price feed for this contract — only the issuer-verified tokenized equities carry price, market cap and 24h volume on this indexer${
        pool
          ? v4?.status === "none"
            ? ", no Uniswap v3 pool against a verified quote asset was found for it, and the Uniswap v4 PoolManager has never emitted an Initialize event naming it either — so BOTH venues were read and neither has a market for it"
            : ", and no Uniswap v3 pool against a verified quote asset was found for it either"
          : ""
...
function startPoolPrice(value, t, o) {
  // NOT STARTED AT ALL when the request has less time left than the read needs. The
  // caller distinguishes this from a read that failed — see priceAvailability's
  // `poolSkipped` — because "we did not look" and "we looked and could not see" are
  // different facts and only one of them is about the chain.
  if (outOfTimeFor(POOL_MIN_MS)) {
    noteBudgetSkip("poolPrice");
    return null;
  }
  return attempt(() =>
    withDeadline(
      ...
      // Its own 14s budget, or whatever the request has left, whichever is smaller.
      clampTimeout(o.deadlines.pool),
      "pool price",
    ),
  );
}
...
function stockBlock(match, resolved, token = null) {
  ...
  return {
    ...
    //
    // ECHOED FROM THE MEASUREMENT ON BOTH PATHS, and no longer the constant "uniswap_v3".
    // That constant was true while v3 was the only reader and became a mislabel the moment
    // a band depth could also come from v4 — and this field's entire purpose is to say
    // which instrument a figure came from, so a wrong value here is worse than none. Null
    // means the venue was not recorded, which is not a claim that it was v3.
    depthSource:
      quoteLiquidityUsd === null
        ? null
        : pooled
          ? (token.pool?.source ?? null)
          : (probed?.depthSource ?? null),
  };
}
...
async function tokenEvidence(value, o, probe, enrich) {
  ...
  // `poolWanted` and `poolStarted` are two different things now: the read can be
  // wanted and not started, because the clock ran out (startPoolPrice returns null
  // for that). Kept apart so the price status below can say which.
  let poolWanted = Boolean(token) && finiteOrNull(token?.exchange_rate) === null;
  let poolStarted = poolWanted ? startPoolPrice(value, token, o) : null;
  ...
  if (!poolWanted && finiteOrNull(t.exchange_rate) === null) {
    poolWanted = true;
    poolStarted = startPoolPrice(value, t, o);
  }
  // Wanted, never started: the clock, not the chain. src.miss names it in
  // `unavailable` like any other missing source, and priceAvailability gives it the
  // sentence that says it was skipped rather than attempted.
  const poolSkipped = poolWanted && !poolStarted;
  if (poolSkipped) src.miss("poolPrice");
  ...
  const availability = priceAvailability(t.exchange_rate, Boolean(token), pool, poolOutage, poolSkipped);
