...
export function buildGroqEvidence({ address, score, inspect, risk }) {
  ...
  if (fg?.status === "attached") {
    ...
    if (shared.length > 0) {
      ...
      fundingOverlap = null; // No shared funders — omit entirely so model cannot hallucinate
    }
  ...
  const evidence = {
    ...
    fundingGraph: (() => {
      const fg = score?.fundingGraph;
      if (!fg) return { status: "not_attached" };
      // Strip sharedInboundFunders if empty to prevent hallucination
      const shared = Array.isArray(fg.sharedInboundFunders) ? fg.sharedInboundFunders : [];
      if (shared.length === 0) {
        return { ...fg, sharedInboundFunders: [], _note: "NO_SHARED_FUNDERS_DO_NOT_ASSIGN_THIS_SIGNAL" };
      }
      return fg;
    })(),
    accountAge,
  ...
  const ARRAY_CAPS = {
    ...
    topPrograms: 8,
    drivers: 12,
    signatures: 24, // keep in sync — Dashboard.jsx groqEvidence signatures slice
    walletAges: 20,
    feePayers: 16,
    entityLedger: 56,
  };
...
function pickSuspiciousSignatures(inspect) {
  if (!inspect || typeof inspect !== "object" || !Array.isArray(inspect.signatures)) return [];
