...
/**
 * EVERY OUTBOUND LINK A PAGE MAKES, from the RAW HTML, for exactly the reason
 * extractAddressMentions reads the raw HTML: THE THING IS IN THE HREF, AND STRIPPING
 * REMOVES HREFS.
 *
 * MEASURED, and it is the same failure in a new place. A site that links its source
 * repository writes `<a href="https://github.com/owner/repo">GitHub</a>` — the anchor TEXT
 * is the word "GitHub" and the repository is in the attribute. Read from the stripped prose
 * alone, such a page mentions no repository at all. Live against https://htmx.org/: the
 * page links its repository and the stripped text contains no github.com URL anywhere, so a
 * reader working from the prose concluded the repository had not been named — and then any
 * repository it went on to look at would have been one nobody named.
 *
 * OPT-IN, and that is deliberate rather than timid. A caller that cannot FOLLOW a link has
 * no use for a list of them, and the ask route's evidence budget (lib/ask-loop.js
 * MAX_EVIDENCE_CHARS, shared with every other tool result in the same round) is not
 * somewhere to spend characters on data nobody reads. The deep-research loop is the only
 * caller that acts on these, so it is the only one that asks — see analyzeSite's
 * `collectLinks`.
 *
 * NOT A LIST OF THINGS TO FETCH. Whoever wrote the page chose what is in it, so every entry
 * is a CANDIDATE that a caller must screen from scratch, and anything reached through one is
 * weaker evidence than something a user gave or the chain declared. See
 * services/research/lib/targets.js.
 *
 * PURE.
 *
 * @param {unknown} html
 * @param {unknown} [finalUrl] - the page's own URL, so first-party links can be labelled
 * @returns {Array<{ url: string, host: string, firstParty: boolean, text: string|null }>}
 */
export function extractLinks(html, finalUrl) {
  const source = typeof html === "string" ? html : "";
  let ownHost = null;
  try {
    ownHost = new URL(String(finalUrl)).hostname.toLowerCase().replace(/^www\./, "");
  } catch {
    ownHost = null;
  }

  const seen = new Map();
  for (const m of source.matchAll(/<a\b([^>]{0,600})>([\s\S]{0,200}?)<\/a\s*>/gi)) {
    const href = m[1].match(/\bhref\s*=\s*["']([^"'\s]{4,400})["']/i)?.[1];
    if (!href || !/^https?:\/\//i.test(href)) continue;
    let url = null;
    try {
      url = new URL(stripJunk(decodeEntities(href)));
    } catch {
      continue;
    }
    const host = url.hostname.toLowerCase().replace(/\.$/, "");
    // The fragment is dropped and the rest is kept: `#install` names a place on a page, not
    // a different page, and keeping it would fill the cap with one document's own anchors.
    url.hash = "";
    ...
    seen.set(key, {
      url: key.slice(0, 400),
      host,
      firstParty: Boolean(ownHost) && (bare === ownHost || bare.endsWith(`.${ownHost}`)),
      // The anchor's own words, which are how a page SAYS what it is linking — "GitHub",
      // "Docs", "Audit". Untrusted like everything else on the page.
      text: flattenUntrusted(decodeEntities(m[2].replace(/<[^>]*>/g, " ")), 80),
    });
    if (seen.size >= LINK_LIMIT) break;
  }
  return [...seen.values()];
}

/** How many outbound links travel. Enough for a navigation bar and a footer, not a sitemap. */
const LINK_LIMIT = 40;

/* ==================== a shell, or the page a visitor sees ==================== */

/**
 * The bounds of the shell test, and every one of them is measured.
 *
 * TEXT_CHARS 120 AND HTML_BYTES 1500. Measured through this module's own stripToText:
 * https://eska.fun/ answers HTTP 200 with 5,782 bytes of HTML carrying FOUR characters
 * of visible text — "ESKA". Four characters is not a thin site; it is a read of a
 * different document than the one anybody looked at. 120 is well above that and well
 * below any page that has actually said something, and the byte floor stops a genuinely
 * tiny page — a redirect stub, a bare "hello" — from being called a shell when there was
 * simply nothing to paint.
 *
 * MARKER_TEXT_CHARS 400 is the looser gate that applies only when a framework mount
 * point is actually present in the markup. Measured: www.ponsfamily.com serves 45,067
 * bytes carrying 603 characters, and renders to 6,475 — real text, and a tenth of the
 * page. So 603 clears this bar and is NOT called a shell: what was read there is real,
 * and the reading says separately that it may be a fraction of the whole.
 */
export const SHELL_LIMITS = Object.freeze({
  TEXT_CHARS: 120,
  HTML_BYTES: 1_500,
  MARKER_TEXT_CHARS: 400,
});

/**
 * Markup that means "a framework fills this in the browser".
 *
 * These are the mount points and boot payloads of the mainstream client-side
 * frameworks. NONE OF THEM IS SUSPICIOUS — this is how an enormous share of the web is
 * built, and the only thing a match establishes is that an HTTP GET is reading the
 * scaffolding rather than the page.
 */
const SPA_MARKERS = Object.freeze([
  { re: /<div[^>]{0,200}\bid\s*=\s*["'](?:root|app|__next|___gatsby|__nuxt|svelte)["'][^>]{0,200}>\s*<\/div>/i, what: "an EMPTY framework mount element (<div id=…></div>) — the page's content is inserted into it in the browser" },
  { re: /<div[^>]{0,200}\bid\s*=\s*["'](?:root|app|__next|___gatsby|__nuxt|svelte)["']/i, what: "a framework mount element (id=root/app/__next/___gatsby/__nuxt/svelte)" },
  { re: /\bdata-reactroot\b|\bdata-react-helmet\b/i, what: "React's own server-render marker" },
  { re: /__NEXT_DATA__|window\.__NUXT__|window\.__INITIAL_STATE__|window\.__APOLLO_STATE__/i, what: "a boot payload the page's JavaScript reads to build itself (__NEXT_DATA__/__NUXT__/__INITIAL_STATE__)" },
  { re: /(?:enable|turn\s+on)\s+JavaScript\s+to\s+(?:run|use|view)/i, what: "the standard <noscript> notice that this application requires JavaScript" },
  { re: /<app-root\b|\bng-(?:app|version)\b/i, what: "an Angular application root" },
  { re: /<div[^>]{0,200}\bid\s*=\s*["']q-app["']|\bdata-v-app\b/i, what: "a Vue application root" },
]);

/**
 * WHETHER A FETCHED DOCUMENT IS THE PAGE OR THE SCAFFOLDING FOR IT.
 *
 * THE FAILURE THIS EXISTS TO STOP, and it is the worst one this feature can commit: a
 * client-rendered site is fetched, four characters come back, and the analysis proceeds
 * as though four characters is what the site says. That is an UNREAD PAGE PRESENTED AS A
 * READ ONE — every downstream observation ("the page makes no claims", "the contract
 * address is not on the page", "the site says almost nothing") is then a finding about a
 * document nobody looked at, attributed to somebody's project.
 *
 * TWO SEPARATE ANSWERS, because they are two different facts:
 *
 *   isShell        — what was fetched carries essentially no text. Nothing about the
 *                    page's content was learned, and the text must NOT be read as what
 *                    the site says.
 *   clientRendered — the markup carries a framework mount point. The text that WAS read
 *                    is real, and may be a fraction of what a visitor sees.
 *
 * NEITHER IS A FINDING ABOUT THE PROJECT. Being built as a single-page application is
 * how a very large share of modern sites are built, including most honest ones, and a
 * site that paints itself in the browser is hiding nothing by doing so.
 *
 * PURE.
 *
 * @param {{ html?: unknown, textChars?: number, bytes?: number }} input
 * @returns {object}
 */
export function shellVerdict({ html, textChars, bytes } = {}) {
  const source = typeof html === "string" ? html : "";
  const chars = Number.isFinite(textChars) ? textChars : 0;
  const htmlBytes = Number.isFinite(bytes) && bytes > 0 ? bytes : source.length;
  const markers = [];
  for (const m of SPA_MARKERS) {
    if (m.re.test(source)) markers.push(m.what);
    if (markers.length >= 4) break;
  }

  const emptyAgainstBody = chars < SHELL_LIMITS.TEXT_CHARS && htmlBytes >= SHELL_LIMITS.HTML_BYTES;
  const clientRendered = markers.length > 0;
  const isShell = emptyAgainstBody || (clientRendered && chars < SHELL_LIMITS.MARKER_TEXT_CHARS);
  // Characters of prose per kilobyte of HTML. A number, not a verdict — it is in the
  // output so a reader can see the gap rather than take "shell" on trust.
  const charsPerKb = htmlBytes > 0 ? Math.round((chars / (htmlBytes / 1024)) * 10) / 10 : null;

  const why = [
    emptyAgainstBody ? `${chars} characters of visible text in ${htmlBytes} bytes of HTML` : null,
    clientRendered ? markers[0] : null,
  ].filter(Boolean);
  ...
  return {
    ...
    charsPerKb,
    reading: isShell
      ? `WHAT THE SERVER SENT IS A SHELL, NOT THE PAGE — ${why.join("; ")}. A document like this is painted in the browser by JavaScript that an HTTP GET never runs, so the text read from it is NOT what a visitor sees and must NOT be read as what the site says. Anything absent from it is UNREAD, not absent. This is ORDINARY: a very large share of the web is built this way, and it is not a finding about the project.`
      : clientRendered
        ? `This page carries the marks of a client-rendered application (${markers[0]}) AND served ${chars} characters of text. What was read is real, and it may be a fraction of what a visitor sees — a browser running the page's own JavaScript would very likely show more. Anything missing from the text below is UNREAD rather than absent.`
        : `The server sent the page's text itself: ${chars} characters in ${htmlBytes} bytes of HTML, with none of the marks of a JavaScript-painted shell. What was read is what the server sends to anyone. A browser might still add to it.`,
    note: "A test of HOW THE PAGE IS BUILT, not of whether it is honest. Server-rendered, client-rendered and static are all ordinary choices, and none of them is evidence about the people behind a project.",
  };
}
...
export async function analyzeSite(url, options = {}) {
  ...
  /**
   * IS THIS THE PAGE, OR THE SCAFFOLDING FOR IT — computed here, where the raw HTML is,
   * and carried on the result so no caller has to guess.
   *
   * Every observation below this line is about the bytes the server sent. When those
   * bytes are a shell, saying so is the difference between "the site says almost
   * nothing" — a finding about somebody's project — and "the page was not read".
   */
  const pageShell = shellVerdict({ html, textChars: stripped.textChars, bytes: first.bytes });
  ...
  return {
    ...
    /* ---- the page, or the scaffolding for it ---- */
    shell: pageShell,
    /* ---- what it says, fenced ---- */
    content: {
      ...
      /**
       * WHERE THIS TEXT CAME FROM, on the same object as the text.
       *
       * "server_html" means an HTTP GET's bytes and nothing more. A caller that renders
       * the page in a browser replaces the text and says so by setting this to
       * "rendered_dom" — see lib/project-profile.js applyRender. A reader must never
       * have to infer which of the two they are quoting.
       */
      textSource: "server_html",
      text: stripped.text,
      ...
      /**
       * THE PAGE'S OUTBOUND LINKS, only for a caller that asked.
       *
       * OPT-IN because a caller that cannot follow a link has no use for a list of them, and
       * this blob shares lib/ask-loop.js MAX_EVIDENCE_CHARS with every other tool result in
       * the round. The deep-research loop is the only caller that acts on these. See
       * extractLinks for the measurement that made them necessary at all.
       */
      ...(options.collectLinks === true ? { links: extractLinks(html, first.finalUrl) } : {}),
    ...
    /* ---- how old the name is ---- */
    domain,
    notChecked: notCheckedList({ fetched: true, checkedDomain: Boolean(domain && domain.status === "measured"), shell: pageShell.isShell }),
    disclaimer:
 ...
 */
/**
 * The `notChecked` entry for a page whose content never arrived because it is painted
 * by JavaScript.
 *
 * EXPORTED SO IT CAN BE REMOVED. When a caller goes on to render the page in a real
 * browser the sentence stops being true, and lib/project-profile.js applyRender drops it
 * by identity rather than by matching on prose that could drift.
 */
/**
 * The `notChecked` entry for a page that was never fetched — and which stops being true
 * the moment a browser reads the same URL. Dropped by identity in the same way.
 */
export const PAGE_NOT_FETCHED =
  "The page itself — nothing was fetched, so every observation about the site is absent rather than negative.";

export const SHELL_NOT_CHECKED =
  "What this page actually shows a visitor. The document fetched is a SHELL painted by JavaScript, which an HTTP GET does not run — so the text below is the scaffolding, not the page, and its emptiness is a fact about how the site is built rather than about what the site says.";

function notCheckedList({ fetched, checkedDomain = false, shell = false }) {
  const items = [
  ...
  if (shell) items.unshift(SHELL_NOT_CHECKED);
  if (!fetched) items.unshift(PAGE_NOT_FETCHED);
