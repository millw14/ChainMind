...
import { resolvePool } from "./dex-price.js";
 ...
 *     statistic. The pool is identified by lib/dex-price.js resolvePool, the burn
 *     address by being one of the two unspendable sinks (see DEAD_ADDRESS — the
 *     live top ten uses 0x0000…dEaD, not the zero address) and the contract by
 *     being the token: three rules, no hardcoded list of addresses to go stale.
 ...
 *
 * @param {string} token
 * @param {{ client?: object, calls?: object, resolvePool?: Function }} [options]
 * @returns {Promise<{ pools: Set<string>, status: "resolved"|"none"|"unread", reason: string|null }>}
 */
export async function tokenPoolAddresses(token, options = {}) {
  ...
  const res = await attempt(() =>
    resolver(token, { client: options.client, calls: options.calls }),
  );
  if (!res.ok || !res.value || typeof res.value !== "object") {
    return { pools: new Set(), status: "unread", reason: "the pool lookup did not complete" };
  }
  ...
  if (addresses.size) return { pools: addresses, status: "resolved", reason: null };
  // The chain answered and there is nothing there — the one case where an
  // unlabelled row can be relied on not to be a pool.
  if (reason === "no_pool") return { pools: addresses, status: "none", reason: null };
  return {
    pools: addresses,
    status: "unread",
    reason:
...
/**
 * What one address IS, relative to this token. Three rules, no address list.
 *
 ...
 */
function classifyRole(address, { token, pools }) {
  if (address === token) {
    return { role: HOLDER_ROLES.CONTRACT, reason: "this is the token contract's own address" };
  }
  ...
  if (pools.has(address)) {
    return { role: HOLDER_ROLES.POOL, reason: "the token's Uniswap v3 pool — this balance is liquidity, not a position" };
  }
  return { role: HOLDER_ROLES.HOLDER, reason: null };
}
 ...
 *   resolvePool?: Function, now?: number, timeoutMs?: number, concurrency?: number }} [options]
 ...
 *   poolReason: string|null, pools: Array<string>, unavailable: Array<string>,
 *   error?: string }>}
 */
export async function holderFirstAcquisition(token, options = {}) {
  ...
  const holders = ranked.map((h, i) => {
    const { role, reason: roleReason } = classifyRole(h.address, { token: tokenAddress, pools: pool.pools });
    return {
      rank: i + 1,
      address: h.address,
      role,
