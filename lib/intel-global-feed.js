...
export async function buildGlobalIntelFeed(client, opts = {}) {
  ...
  let pairSql = `
    SELECT scope_a, scope_b, COUNT(*) AS shared_wallets,
  ...
  if (lookbackHours != null) {
    pairSql += ` WHERE lookback_hours = ?`;
    pairArgs.push(lookbackHours);
  }
  pairSql += `
  ...
  try {
    const pairs = await client.execute({ sql: pairSql, args: pairArgs });
