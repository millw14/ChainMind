...
export async function gatherEvidence(target) {
  ...
  if (kind === "address") {
    ...
    const isToken = Boolean(addr.token) || (addr.is_contract === true && addr.token !== undefined);
...
function sumShares(holders) {
  const total = holders.reduce((acc, h) => acc + (Number(h.share) || 0), 0);
  return holders.length ? round2(total) : null;
}
