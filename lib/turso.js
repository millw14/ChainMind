...
function withRetry(client) {
  ...
  const RETRYABLE = /connections? limit|reduce concurrency|SQLITE_BUSY|timeout|timed out|stream not found|HeadersTimeout|SERVER_ERROR|HTTP status 5|\b50[234]\b|fetch failed|ECONNRESET|UND_ERR_SOCKET|other side closed|socket|terminated|ECONNREFUSED|EPIPE/i;
...
/** Build a retrying libSQL client for an explicit URL/token (e.g. backups of the
 *  self-hosted instance over a flaky link). */
export function makeRetryingClient(url, authToken) {
  if (!url || !authToken) return null;
  return withRetry(createClient({ url: String(url).replace("libsql://", "https://"), authToken: String(authToken) }));
}
...
/**
 * Cheap check: does this scope have ANY events indexed (regardless of time window)?
 * Used to distinguish a genuinely cold scope (worth a slow live-RPC score) from one
 * that's indexed but whose activity is just outside the requested lookback.
 * @param {import("@libsql/client").Client} client
 * @param {string} scope
 */
export async function tursoScopeHasAnyEvents(client, scope) {
  const result = await client.execute({
    sql: "SELECT 1 FROM events WHERE scope_address = ? LIMIT 1",
    args: [scope],
  });
 ...
 * @param {number} [limit]
 */
export async function tursoFetchSurfaceHits(client, limit = 48, offset = 0) {
  const result = await client.execute({
    sql: `
      ...
      LIMIT ? OFFSET ?
    `,
    args: [Math.min(200, Math.max(1, limit)), Math.max(0, Number(offset) || 0)],
  });
  return result.rows.map((row) => {
    let entities = [];
    try {
...
/**
 * Score cache read — returns the cached bundle if it exists and is younger than ttlSec.
 * @param {import("@libsql/client").Client} client
 */
export async function tursoFetchScoreCache(client, scope, windowMinutes, lastHours, ttlSec) {
  try {
    const res = await client.execute({
      sql: "SELECT payload_json, created_at FROM score_cache WHERE scope_address = ? AND window_minutes = ? AND last_hours = ?",
      args: [scope, windowMinutes, lastHours],
    });
    const row = res.rows[0];
    if (!row) return null;
    const age = Date.now() / 1000 - Number(row.created_at);
...
/** @param {import("@libsql/client").Client} client */
export async function tursoUpsertScoreCache(client, scope, windowMinutes, lastHours, payload) {
  try {
    await client.execute({
      sql: `INSERT OR REPLACE INTO score_cache (scope_address, window_minutes, last_hours, payload_json, created_at)
            VALUES (?, ?, ?, ?, ?)`,
      args: [scope, windowMinutes, lastHours, JSON.stringify(payload), Math.floor(Date.now() / 1000)],
    });
  } catch {
    /* best-effort */
  }
...
/**
 * Fixed-window rate limit. Returns { allowed, count } for `key` in the current window
 * (default one minute; pass windowSeconds for a coarser bucket). On limiter error the
 * result is fail-open with `error: true` so cost-sensitive callers can choose to block.
 * @param {import("@libsql/client").Client} client
 * @param {string} key client identifier (e.g. IP)
 * @param {number} limit max requests per window
 * @param {number} [windowSeconds] bucket width in seconds (default 60)
 */
export async function tursoRateLimit(client, key, limit, windowSeconds = 60) {
  const win = Math.max(1, Number(windowSeconds) || 60);
  const bucket = `${key}:${Math.floor(Date.now() / (win * 1000))}`;
  try {
    const res = await client.execute({
      sql: `INSERT INTO rate_limit (bucket, n, created_at) VALUES (?, 1, ?)
            ON CONFLICT(bucket) DO UPDATE SET n = n + 1 RETURNING n`,
      args: [bucket, Math.floor(Date.now() / 1000)],
    });
    const n = Number(res.rows[0]?.n ?? 1);
    // Opportunistic prune (~1% of calls): expired buckets are never read again, so
    // drop rows older than an hour to keep the table from growing without bound.
    if (Math.random() < 0.01) {
      client
        .execute({
          sql: "DELETE FROM rate_limit WHERE created_at < ?",
          args: [Math.floor(Date.now() / 1000) - 3600],
        })
        .catch(() => {});
    }
    return { allowed: n <= limit, count: n };
  } catch {
    return { allowed: true, count: 0, error: true }; // fail-open: never block on limiter error
  }
}

export async function tursoFetchRecentCases(client, limit = 20, offset = 0) {
  // Cap covers the largest caller page (+1 hasMore probe row); routes clamp tighter.
  const lim = Math.min(501, Math.max(1, Number(limit) || 20));
