...
export async function measureDepths(tier, options = {}) {
  ...
  const bound = Number.isInteger(options.bound) && options.bound > 0 ? options.bound : MAX_DEPTH_PROBES;
  const probe = typeof options.probe === "function" ? options.probe : probeQuoteDepth;

  const shortlist = depthShortlist(rows, bound);
...
export function collisionNotice(v, symbol) {
  ...
  const dropped =
    v.dropped > 0
      ? ` ${v.dropped} more went unmeasured — the probe is capped at ${v.bound} pools per lookup, so they are unknown rather than shallow.`
      : "";
