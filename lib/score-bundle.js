...
export async function buildTursoScoreBundle(client, p, opts = {}) {
  ...
  if (
    ...
    // These depend only on the wallet-node list. Run them at LOW concurrency (2 at a
    // time) not all-at-once: Turso's free tier caps concurrent connections, and the
    // ingest worker is also using some — firing 4-5 in parallel here (plus baseline)
    // blew the cap → "connections limit exceeded" → score 504 → dashboard "NO DATA".
    // On Vercel's low latency each query is ~100-200ms, so 2-wide is still fast.
    const [edgeRows, trows] = await Promise.all([
      payers.length > 0
        ? tursoFetchInboundFundingEdges(client, scope, payers, cutoff).catch(() => [])
        : Promise.resolve([]),
      tursoFetchTransfersWindow(client, scope, cutoff, 2500).catch(() => []),
    ]);
    const [prow, ageRows] = await Promise.all([
