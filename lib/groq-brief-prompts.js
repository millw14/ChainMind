 ...
 * 2. Evidence — serialized `userEvidence` JSON only, under a fixed header (API `user` message, block 1).
...
export const GROQ_BRIEF_INSTRUCTION_PROMPT = `Apply the system rules to the Evidence JSON above. Every claim in your output must be grounded in fields present in Evidence — do not invent funding paths, account ages, or transactions not shown.
...
/**
 * Build the full `user` message: evidence block + instruction block + optional focus.
 ...
 */
export function buildGroqBriefUserContent(evidencePrettyJson, focusLine) {
  const parts = [
    GROQ_BRIEF_EVIDENCE_SECTION_TITLE,
    evidencePrettyJson.trim(),
    "",
